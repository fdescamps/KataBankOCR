...
CheckSumStream.prototype._transform = function ( solution, encoding, processed ) {
    ...
    */
    ...
    this.push( solution.join('') );//when you push just a basic false the stream ends... WTF????
    processed();
...
util.inherits(WriteInFileStram, Transform);
function WriteInFileStram () {
  Transform.call(this, { "objectMode": true });
  this.nbNumbers = 0;
}

WriteInFileStram.prototype._transform = function ( checked, encoding, processed ) {
  var wsstream = fs.createWriteStream( './out/katabankOCR-data-output-'+ this.nbNumbers + '.js' );
  wsstream.write( checked );
  wsstream.end();
